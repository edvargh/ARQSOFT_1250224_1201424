...
/**
 * Opaque-box integration tests for GenreController against a real SQL backend.
...
@Transactional
class GenreControllerSqlIT extends SqlBackedITBase {
  ...
  private Book book(String isbn, String title, String desc, Genre g, List<Author> authors) {
    var b = new Book(isbn, title, desc, g, authors, null);
  ...
  void top5_404_whenNoBooks() throws Exception {
    mvc.perform(get("/api/genres/top5"))
  ...
  void top5_200_withBookCounts_sorted() throws Exception {
    ...
    mvc.perform(get("/api/genres/top5"))
        ...
        .andExpect(jsonPath("$.items[1].bookCount").value(1));
  }
